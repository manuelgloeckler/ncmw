...
def run_analysis(cfg : DictConfig) -> None:
    ...
    for model, df in zip(models,dfs):
        sol = model.optimize()
        df["flux"] = sol.fluxes
        df.to_csv(PATH + SEPERATOR + "flux_analysis" + SEPERATOR + "fva_" + model.id + ".csv")
        log.info(df)

    log.info("Computing COMPM media for modles")
    mediums = compute_COMPM(models,dfs)
    for model, medium in zip(models,mediums):
        with open(PATH + SEPERATOR + "medium" + SEPERATOR + "COMPM_" + model.id + ".json", "w") as f:
            json.dump(medium, f)

    log.info("Plotting fva results")
    for model, df in zip(models,dfs):
        plot_full_fva(df,PATH + SEPERATOR + "flux_analysis" + SEPERATOR + "full_fva_plot" + model.id + ".pdf")
        plot_medium_fva_range(model, PATH + SEPERATOR + "flux_analysis" + SEPERATOR + "medium_fva_plot" + model.id + ".pdf",cfg.analysis.fva_fraction)

    log.info("Computing Jacard Similarities")
    df_met, df_rec, df_ro = jaccard_similarity_matrices(models)
    df_met.to_csv(PATH + SEPERATOR + "similarity" + SEPERATOR + "jacard_similarities_metabolies.csv")
    df_rec.to_csv(PATH + SEPERATOR + "similarity" + SEPERATOR + "jacard_similarities_reactions.csv")
    df_ro.to_csv(PATH + SEPERATOR + "similarity" + SEPERATOR + "jacard_similarities_exchanges.csv")
