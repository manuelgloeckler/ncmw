...
class Model:
    ...
    def __add__(self, model2):
        ...
        return CommunityModel([self, model2], [1.0, 1.0])
...
class CommunityModel(Model):
    ...
    def _weighted_objective(self, weight):
        ...
        for i, model in enumerate(self.models):
            self.objective_c = np.append(
                self.objective_c, weight[i] * model.objective_c
            )
...
class MIP_community_model:
    def __init__(self, model1, model2, weights=[1.0, 1.0]):
    ...
    def build_mip_model(self):
        ...
        for i in range(self.S1.shape[0]):
            self.comm_model.add_constr(
                xsum(self.S1[i, j] * x1[j] for j in range(self.S1.shape[1])) == 0
            )

        for i in range(self.S2.shape[0]):
            self.comm_model.add_constr(
                xsum(self.S2[i, j] * x2[j] for j in range(self.S2.shape[1])) == 0
            )
    ...
    def compute_coopm(self, enforce_survival=True):
        ...
        if enforce_survival:
            ...
            self.comm_model.add_constr(
                self.weights[0] * self.x1[self.obj1]
                + self.weights[1] * self.x2[self.obj2]
                >= minMBR
            )
    ...
    def optimize(self):
        self.comm_model.objective = maximize(
            self.weights[0] * self.x1[self.obj1] + self.weights[1] * self.x2[self.obj2]
        )
        self.comm_model.optimize()
        return (
            self.weights[0] * self.x1[self.obj1].x
            + self.weights[1] * self.x2[self.obj2].x
        )
    ...
    def compute_alpha_weightings(self, alpha, maxbiomass=0.1):
        ...
        # Alpha objective...
        self.comm_model.add_constr(
            self.weights[0] * self.x1[self.obj1] <= alpha * maxbiomass
        )
        self.comm_model.add_constr(
            self.weights[1] * self.x2[self.obj2] <= (1 - alpha) * maxbiomass
        )
    ...
    def compute_alpha_coopm(self, alpha, maxbiomass=0.1, enforce_survival=True):
        ...
        if enforce_survival:
            self.comm_model.add_constr(
                self.weights[0] * self.x1[self.obj1] >= alpha * minMBR
            )
            self.comm_model.add_constr(
                self.weights[1] * self.x2[self.obj2] >= (1 - alpha) * minMBR
            )
        else:
            self.comm_model.add_constr(
                self.weights[0] * self.x1[self.obj1]
...
def optimize_coopm_community(
    model, MBR, biomass_reactions, weights, enforce_survival=True
):
    model = model.copy()
    minMBR = 0.1 * MBR
    ...
    if enforce_survival:
        constraint_growth = [
            model.problem.Constraint(f, lb=minMBR, ub=1000)
            for i, f in enumerate(biomass)
        ]
    else:
        constraint_growth = model.problem.Constraint(sum(biomass), lb=minMBR, ub=1000)
